...
def anime_text(text, delay=0.03):    
    for ch in text:    
        print(Fore.RED + ch, end="", flush=True)    
        time.sleep(delay)    
...
banner()    
anime_text("[+] أدخل البيانات المطلوبة ↓")    

TOKEN = input(Fore.RED + "\n[•] أدخل التوكن: ")    
CHANNEL = input(Fore.RED + "[•] أدخل ايدي القروب: ")    

# واجهة الخيارات    
anime_text("\n[1] إرسال رسائل متكررة")    
anime_text("[2] تبديل الاسماء بالتناوب")    
choice = input(Fore.RED + "[•] اختر الخيار: ")    
...
if choice == "1":    
    count = int(input(Fore.RED + "[•] كم رسالة تريد إرسالها بالتناوب؟ "))    
    for i in range(count):    
        msg = input(Fore.RED + f"[+] أدخل الرسالة رقم {i+1}: ")    
        MESSAGES.append(msg)    
    DELAY = float(input(Fore.RED + "[•] أدخل التأخير بين كل رسالة بالثواني: "))    
elif choice == "2":    
    count = int(input(Fore.RED + "[•] كم اسم تريد تغييره بالتناوب؟ "))    
    for i in range(count):    
        name = input(Fore.RED + f"[+] أدخل الاسم رقم {i+1}: ")    
        NAMES.append(name)    
    DELAY = float(input(Fore.RED + "[•] أدخل التأخير بين كل تغيير بالثواني: "))    
else:    
